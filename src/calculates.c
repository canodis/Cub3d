...
/*   Updated: 2022/10/16 17:26:01 by rtosun           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	calculate_ray(t_game *game, int x)
{
	...
	game->ray->ray_dirx = game->pdata->dir_x
		+ (game->pdata->plane_x * game->ray->camera_x);
	game->ray->ray_diry = game->pdata->dir_y
		+ (game->pdata->plane_y * game->ray->camera_x);
...
void	calculate_steps(t_game *game, int *step_x, int *step_y)
{
	...
	{
		*step_x = -1;
		game->ray->side_distx = (game->pdata->pos_x - game->ray->mapx)
			* game->ray->delta_distx;
	}
	else
	{
		*step_x = 1;
		game->ray->side_distx = (game->ray->mapx + 1.0 - game->pdata->pos_x)
			* game->ray->delta_distx;
	}
	if (game->ray->ray_diry < 0)
	{
		*step_y = -1;
		game->ray->side_disty = (game->pdata->pos_y - game->ray->mapy)
...
void	calculate_textures(t_game *game, int side)
{
	...
	game->draw->tex_pos = (game->draw->draw_s - SCREEN_HEIGHT / 2
